...
function apply() {
  emit("update:modelValue", invited.value)
  dropdownOpen.value = false
}
...
function selectAll() {
  invited.value = (props.options || []).map(o => o.id)
}

function removeInvited(id) {
  invited.value = invited.value.filter(x => x !== id)
  emit("update:modelValue", invited.value) // immediate remove, matches your screenshot feel
}
...
// close dropdown when clicking outside the dropdown panel (NOT when clicking inside modal)
...
<template>
  <teleport to="body">
    <div v-if="open" class="fixed inset-0 z-[9999]">
      ...
      <div class="mx-auto max-w px-4 py-5">
        ...
        <div class="mt-6" ref="panelRef">
          ...
          <div v-if="dropdownOpen" class="mt-2 rounded-2xl bg-white ring-1 ring-black/10 shadow-lg overflow-hidden">
            ...
            <div class="flex items-center justify-between border-t px-4 py-3 text-sm">
              <button class="text-black/60" @click="selectAll">Select all</button>
              <button class="text-black/60" @click="reset">Reset</button>
              <button class="font-semibold text-black" @click="apply">Apply</button>
            </div>
          </div>
        </div>
<!-- currently invited list -->
